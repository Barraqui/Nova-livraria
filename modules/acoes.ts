import { devolverLivro, emprestarLivro } from "./emprestimos.ts";
import { atualizarLivro, buscarLivrosPorCategoria, buscarLivrosPorId, buscarLivrosPorTitulo, cadastrarLivro, listarLivros, removerLivro } from "./livros.ts";
import { Interface } from "node:readline/promises";
import type { Livro } from "./types.ts";

export async function acaoListar(): Promise<void> {
    const livros = await listarLivros();
    console.log(livros);
}

export async function acaoBuscarPorId(rl: Interface): Promise<void> {
...
export async function acaoBuscarPorCategoria(rl: Interface): Promise<void> {
...
export async function acaoBuscarPorTitulo(rl: Interface): Promise<void> {
...
export async function acaoCadastrarNovoLivro(rl: Interface): Promise<void> {
...
export async function acaoAtualizarLivro(rl: Interface): Promise<void> {
...
export async function acaoRemoverLivro(rl: Interface): Promise<void> {
...
export async function acaoEmprestarLivro(rl: Interface): Promise<void> {
...
export async function acaoDevolverLivro(rl: Interface): Promise<void> {
    const idDigitado = await rl.question("Informe o ID do livro: ");
    const id = Number(idDigitado);
